...
The system is deployed on four different tiers: the Data layer, the Application Layer, the Service layer and the Presentation layer, that are respectively mapped on the four main components of the software application: the data handling logic, the services, the controllers and the client application.
\begin{figure}[!htpb]
    	\centering
...
\subsection{Component View}
    ...
    \paragraph{Model:}
    a set of components representing the data used to build the user interface.
    
    \paragraph{View:}
    a set of components responsible of showing the data related to the respective model.
    
    \paragraph{Google Maps API:}
    used by the client to create maps through Google Maps services.
    
    \paragraph{External Device API:}
    used to connect and get data from the Individual external device from a common smart-watch to a specific pathology monitoring device.
...
The integration phase will start only when the completion of the components involved in the integration should be at least 50\% for the \textit{"services"} and at least 70\% for \textit{"repositories"}: at least critical operations should have been implemented and should have passed the unit test.
