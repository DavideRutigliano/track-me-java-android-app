...
\subsection{Definition, Acronyms and  Abbreviations}
            ...
            \begin{acronym}
                ...
                \acro{MVC}{Model View Controller (design pattern)}
...
The system is deployed on three different tiers: the Persistence layer, the Service layer and the Application layer, that are respectively mapped on the three main components of the software application: the data handling logic, the services, and the controllers.
\begin{figure}[!htpb]
    	\centering
...
The main component is the system itself and includes both the \textit{Web Server} and the \textit{Application Server}. The former communicates in synchronous way over HTTPS with client browser or mobile application, while the latter manages all the application logic and it is interfaced with the \textit{Persistence Layer}. Furthermore, the \textit{Application Layer} can also \textit{consume} REST services made available by the \textit{Application Server}. The \textit{Persistence Layer} instead is composed of the data management logic and is responsible of the communication with the DBMS.

Moreover, both the \textit{Business Logic Layer} and the \textit{Application Layer} should interact with external services:
...
\subsection{Component View}
    ...
    \subsubsection{Presentation Layer (Client)}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=100mm,keepaspectratio]{DD/images/er_diagram.png}
    \caption{ER Diagram}
...
In this design document we assumed the developer is using Java as program-\newline ming language and in particular is deploying both client and server through Spring Framework. The rationale for this choice is that this framework fits very well the architectural style of the proposed software solution, has proprietary implementation of the Model View Controller design patter and of several APIs and is very suitable for both provisioning and consuming RESTful services.

The choice of using Java and Spring is not a full restriction on the deployment process. In such cases the development team would decide to use a different programming language or to not use Spring Framework, due to the proprietary implementation of several classes, such as the persistence API or the Spring MVC ones, some adaptions to the class diagram may be needed.
...
Due to the multi-tier architecture style, the different \textit{Presentation}, \textit{Application}, \textit{Domain} and \textit{Data} layers of the application could be developed separately and may be developed in parallel. With respect to specific application services, the implementation plan to be followed should be to develop first \textit{Data4Help} functions and then \textit{Automated-Sos} and \textit{Track4Run} facilities, which are built on top of \textit{Data4Help} service. \textit{Authentication} subsystem instead can be implemented independently on the order on which other components are; an implementation strategy for this module is to deploy it in parallel with Data4Help subsystem.
...
The test should cover 100\% of all test cases that involve components of the business logic and persistence layer and at least 80-90\% for the application layer. The testing phase will be suspended in case such percentages are lower than 50\% for the application and data logic and lower than 30\% for application layer. Conditions to be satisfied listed below:

\begin{itemize}
    \item all test cases must have been executed;
    \item 90-100\% coverage of the requirements and functions has been achieved;
    \item 70-80\% coverage of the source code has been achieved;
    \item all high severity or top priority bug has been fixed;
    \item high risk identified area has been taken up and tested.
\end{itemize}

If \textit{I\&T} exit criteria are satisfied, the test phase can move on to the System Testing in which Load, Performance and Stress tests may be useful to highlight some Quality of Service leaking of the system.
