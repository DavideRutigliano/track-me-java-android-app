...
\begin{itemize}
    \item \textbf{(REQ-18)} The system must allow the Third-Party to activate Automated-\newline SOS service;
    \item \textbf{(REQ-19)} The system must be able to assign an ambulance dispatcher to the SOS service;
    \item \textbf{(REQ-20)} The system must allow the Individual to activate Automated-\newline SOS service;
...
\begin{enumerate}
    ...
    \item In the tab \textit{Privileges} check \textit{Superuser} and \textit{Can login?}, then Save.
\end{enumerate}

You should have the above configuration at the end:
\begin{figure}[!htpb]
    \centering
    \includegraphics[height=73mm]{ITD/images/postgresql_install.png}
\end{figure}
\newpage
...
\begin{enumerate}
    ...
    \item Install android studio and
    \href{https://developer.android.com/studio/run/managing-avds}{\underline{Android Virtual Device}};
