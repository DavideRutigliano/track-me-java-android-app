    ...
    \subsection{Functional Requirements}
      ...
      \end{usecase}
       ...
       \end{usecase}
        ...
        \begin{usecase}{Manage Runs}
        \name{Manage Runs}
        \brief{An Individual Selects an option between: enroll a run, unroll run , watch a run.create a run, start a run or delete a run.}
        \actor{Individual.}
        \pre{The Individual is logged-in and has  clicked on \textit{Manage Run} button.}
        \bflow{Track4Run provides three buttons:  \textit{'Enroll Run'}, \textit{'Unroll Run'} ,\textit{Watch Run}, \textit{'Create Run'}, \textit{'Start Run'} and \textit{Delete Run}.}
              {The Individual selects one option}
              \post{The Individual is in the desired section.}
        \end{usecase}
        ...
        \actor{Organizer.}
        \pre{The Individual sas clicked on the \textit{Create Run} button in the \textit{manage run} section.}
        ...
        \actor{Athlete.}
        \pre{The Athlete is logged-in as an individual and has clicked on the \textit{Enroll Run} button in the \textit{Manage run} section, there is at least a run available that has state \textit{created}.}
        ...
        \actor{Athlete.}
        \pre{The Athlete is logged-in as an individual and has clicked on the \textit{Unroll Run} button in the \textit{Manage run} section, he is enrolled to at least one run.}
        ...
        \actor{Spectators.}
        \pre{The Athlete is logged in as an individual and has clicked on the \textit{Watch Run} button in the \textit{Manage run} section. There is at least one run which state is \textit{started} }
