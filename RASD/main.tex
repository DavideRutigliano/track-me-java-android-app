...
\section{Overall Description}
    ...
    In contrast, the \textit{World Domain} is the real-life context in which the \textit{Machine} will be introduced it is that part of the world in which the Machine’s actions will be observed.
...
\section{Specific Requirements}
    ...
    \subsection{Functional Requirements}
        ...
        \bflow{The individual goes on login page pressing \textit{'Sign-in'} button.}
              {TrackMe provides to the \textit{user} a form composed by two text boxes where the user can write and a button to send the form. The first box is the \textit{'username'}, while the the latter is the \textit{'password'} field.}
        ...
        \bflow{The Third-Party presses the \textit{'Enable Automated-SOS'} button.}
              ...
              \post{Third-Party has enabled Automated-SOS and individuals that enabled Automated-SOS can see the TP in the Automated-SOS TP list.}
        \end{usecase}
        ...
        \bflow{The Individual presses the \textit{'Enable Automated-SOS'} button.}
              {Automated-SOS checks if the user has already enabled the service.}
              {Automated-SOS asks if the user wants to enable Automated-SOS. It provides a list of Third parties that enabled Automated-SOS. User can choose only one TP. Automated-SOS also provides two buttons called \textit{'Confirm'} and \textit{'Back'}. The \textit{'Confirm'} button can be pressed only if at least one TP is selected.}
              {Individual selects a TP and presses the \textit{'Confirm'} button.}
        \post{Third-Party has enabled Automated-SOS and individuals that enabled Automated-SOS can see the TP in the Automated-SOS TP list.}
        \end{usecase}
        ...
        \pre{The Third party is logged-in and it's on the main page.}
        \bflow{The Third party presses a button called \textit{'Track4Run'}. TP is now on the Track4Run page.}
