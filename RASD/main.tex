...
\section{Introduction}
    ...
    \subsection{Scope and Audience}
        ...
        To register himself an user must provide an username and a password as well as his/her personal information. To be logged in, the user should send through the login form his credentials which the system will use to authenticate him.
        
        \textit{TrackMe} may further yield an interface that allows individual users to connect external devices and if so, of course a method for acquiring data from it.
        ...
        \begin{description}
            ...
            \item[G.10] Authorize third-party users to get individuals' data if evaluated as anonymous
            \item[G.11] Allow the organizer create a new run
            \item[G.12] Let the organizer delete a run
            \item[G.13] Let the organizer start a new run
            \item[G.14] Permit the athlete enroll to a run
            \item[G.15] Allow the athlete unroll to a run
            \item[G.16] Let the spectator watch a run
        \end{description}
    ...
    \subsection{Definition, Acronyms and  Abbreviations}
            ...
            This document uses several terms that might be more loosely used elsewhere. These terms are defined here as they will be used later on in this document.
                \begin{description}
                    \item[\textbf{Guest}] an external customer of the system that wants to register or login into the application
                    
                    \item[\textbf{User}] a \textit{Guest} customer registered/logged into the application
                    
                    \item[\textbf{Individual}] a single person, registered or logged into the application identified either by SSN or by FC number
                    \item[\textbf{Third-Party}] an organization registered or logged in \textit{TrackMe}, identified by VAT number
...
\section{Overall Description}
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \comment{I think these are "definition" (also better than the ones I've actually written)}
    %maybe move to definition section
        \subsubsection{Data4Help}
        \begin{itemize}
